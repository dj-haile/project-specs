...
### Added
- **Distributable research-agent skills** (`skills/.curated/` + `scripts/build_skills.py`): the six read-only research agents are now published in the vercel-labs/skills community shape (`SKILL.md` with `name` + `description`), installable into 70+ agent tools via `npx skills add dj-haile/project-specs`. `build_skills.py` generates them from the agent sources (preserving `model`/`tools` under `metadata:` for provenance and rewriting relative links to absolute GitHub URLs); a `skills-drift` CI job regenerates and `git diff --exit-code`s them so they can't rot. The framework-coupled commands are intentionally NOT exported (they expect `specs.config.yaml`/`conventions/`); rationale and full design in `thoughts/shared/decisions/2026-08-01-skills-cli-distribution.md`. The `skills/_template` is marked `metadata.internal: true` so the CLI never offers it. `validate.py` now checks every `SKILL.md` carries `name` + `description` (or is marked internal). `--provider=claude` install remains byte-identical (setup.sh never copies source `skills/`).
